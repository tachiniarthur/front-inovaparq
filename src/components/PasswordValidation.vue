    ...
    <ul class="rounded-md w-full">
      <li class="flex items-center py-1" v-for="(validation, index) in validations" :key="index">
        <font-awesome-icon
          ...
          class="text-lg"
        ></font-awesome-icon>
        <span :class="validation.isValid() ? 'text-green-700' : 'text-red-700'" class="font-medium text-sm ml-3">
          {{ validation.message }}
        </span>
      </li>
    </ul>
...
const validations = [
  ...
  },
  {
    ...
    check: () =>
      props.password && props.confirmPassword && props.password === props.confirmPassword
        ? 'circle-check'
        : 'circle-xmark',
